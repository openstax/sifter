window.addEventListener("load", () => {
  ...
  function dom(tagName, attrs = {}, children = [], handlers = {}) {
    ...
    if (typeof tagName === "string") el = document.createElement(tagName);
    else el = tagName;
  ...
  async function fetchWithError(url, useJSON) {
    try {
      ...
      if (useJSON) return await response.json();
  ...
  function getSelectedBookUrls() {
    return Array.from(
      booksEl.querySelectorAll("option:checked"),
      (e) => e.value
    );
  }
  ...
  const getPageUrls = async (format, bookUrl, bookInfo) => {
    if (format === "xhtml") {
      ...
      if (bookJson === null) {
        throw new Error(
          `BUG: Could not find the book even though this URL worked earlier when building the book list ${bookUrl}`
        );
      }
      const pageUuids = [];
      recLeafPages(pageUuids, bookJson.tree);
      const pageUrls = pageUuids.map((pageUuid) => {
        const parseUrl = bookUrl.replace(
          ".json",
          `:${pageUuid.replace("@", "")}.json`
        );
      ...
      for (const collectionXMLUrl of bookUrls) {
        ...
        sandbox.querySelectorAll("[document]").forEach((module) => {
          const parseUrl = new URL(
            relativeHref(
              collectionXMLUrl,
              `../modules/${module.getAttribute("document")}/index.cnxml`
            )
          );
  ...
  const startFn = async () => {
    ...
    for (const bookUrl of selectedUrls) {
      ...
      const pageUrls = await getPageUrls(
        sourceFormat.value,
        bookUrl,
        bookUrlInfoMapping.get(bookUrl)
      ); // Array<{parseUrl, viewUrl}>
      ...
      for (let i = 0; i < pageUrls.length; i++) {
        ...
        const { title, source } = await getPageXML(
          sourceFormat.value,
          parseUrl
        );
        if (!source) {
        ...
        bookLogEl.textContent = `${
          bookUrlInfoMapping.get(bookUrl)?.slug || bookUrl
        } (${i + 1}/${pageUrls.length}. Found ${totalMatches})`;
  ...
  fetch(ablUrl).then(
    async (res) => {
      ...
      if (approvedBookList.api_version !== TESTED_ABL_VERSION) {
        alert(
          `Sifter is tested to work with ABL version ${TESTED_ABL_VERSION} but the current api_version is '${approvedBookList.api_version}'. Stuff may not work`
        );
      }
        ...
        for (const { slug, uuid, commit_sha } of ablBookSlugCommitPairs) {
          if (!loadedBooks.has(slug)) {
            ...
            if (bookJson) {
              ...
              failedBooks.set(slug, {
                url,
                error: "Could not find book in S3",
              });
            }
          }
        }
    ...
    (err) =>
      alert(
        `Problem getting the approved book list. Maybe you are offline or the URL has moved. '${ablUrl}'`
      )
  );

  const compareStrings = (a, b) => (a < b ? -1 : a > b ? 1 : 0);
  const compareKeys = ([a], [b]) => compareStrings(a, b);
  const compareRepoNames = ({ repoName: a }, { repoName: b }) =>
    compareStrings(a, b);
  async function populateBookList() {
    if (sourceFormat.value === "xhtml") {
      const loadedAndFailedBooks = [...failedBooks, ...loadedBooks];
      dom(
        booksEl,
        {},
        loadedAndFailedBooks
          .sort(compareKeys)
          .map(([slug, { url, pipeline, uuid, shortSha, error }]) => {
            bookUrlInfoMapping.set(url, {
              slug,
              url,
              pipeline,
              uuid,
            ...
            if (error)
              return dom("option", { value: url, disabled: true }, [
                `${slug} Error: ${error}`,
              ]);
            else return dom("option", { value: url, selected: true }, [slug]);
          })
      );
      ...
      dom(
        booksEl,
        {},
        loadedRepos.sort(compareRepoNames).map(({ repoName, url, error }) => {
          bookUrlInfoMapping.set(url, { slug: repoName });
          if (error)
            return dom("option", { value: url, disabled: true }, [
              `${repoName} Error: ${error}`,
            ]);
          else return dom("option", { value: url, selected: true }, [repoName]);
        })
      );
    } else {
  ...
  function recLeafPages(acc, node) {
    if (node.contents) {
      ...
      // Autogenerated Pages end with '@{sha}' instead of '@' for some reason
  ...
  function xpathNamespaceResolver(prefix) {
    if (namespacePrefixes[prefix]) {
      ...
      alert(
        'Invalid namespace prefix. Use "h:" or "xhtml:" or "m:" or "mathml:" .'
      );
  ...
  function findNearestId(el) {
    ...
    return (
      ...
      findNearestId(el.parentElement)
    );
  }
